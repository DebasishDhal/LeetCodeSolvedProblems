...
-- We self-join the activity table, for one table the activity is start while for another the activity is end.
-- Beats 65% of users
...
/*
#Sloppy 1st attempt - Beats only 6% and unreliable.
import pandas as pd
...
def get_average_time(activity: pd.DataFrame) -> pd.DataFrame:
    ...
    df = start_df.merge(end_df, on = ['machine_id', 'process_id'], how = 'inner')
    df['processing_time'] = df['timestamp_y'] - df['timestamp_x']
    print(df['processing_time'].abs().agg('mean'))
    df.drop(columns = ['timestamp_y','timestamp_x'],inplace=True)
    df = df.groupby(['machine_id'], as_index=False).agg(processing_time = ('processing_time','mean')).round(3)
    print(df.columns)
    return df

#Much better solution. Concise and smooth.
import pandas as pd
